...
; program 12f675: sudo ./rpp-tlc -w -i timer.hex
; hardware: GPO--1kohm--LED--GND
;
; Configuration:
...
; interrupt vector, general interrupt handling routine goes here
		org	H'04'
; timer1 interrupt, check PIR1 bit 0 
                bcf     STATUS, RP0
                btfsc   PIR1, TMR1IF
                call timerint
                retfie
...
; clear TMR1IF bit
                bcf     PIR1, TMR1IF
; set timer1 interrupt enable bit PIE1 bit 0
		bsf	STATUS, RP0
                bsf     PIE1, TMR1IE 
...
; set GIE bit INTCON bit 7
                bsf     INTCON, GIE
; set timer1 prescaler T1CON bits 4 and 5 to 1:8
                bcf     STATUS, RP0 
                bsf     T1CON, T1CKPS1
                bsf     T1CON, T1CKPS0
